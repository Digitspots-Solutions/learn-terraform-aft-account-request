...
#
# WORKFLOW:
#   1. Copy example.tfvars to terraform.tfvars
#   2. Edit terraform.tfvars with your AWS profile and passwords
#   3. Uncomment and customize module blocks for your domains
#   4. Run: terraform init && terraform apply
#   5. Copy the aft_account_request_snippet output to ../terraform/main.tf
...
variable "aws_profile" {
  ...
  type        = string
  default     = "default"  # Change in terraform.tfvars
}

variable "workmail_region" {
  description = "AWS region for WorkMail (us-east-1, us-west-2, or eu-west-1 only)"
  type        = string
  default     = "us-east-1"

  validation {
    condition     = contains(["us-east-1", "us-west-2", "eu-west-1"], var.workmail_region)
    error_message = "WorkMail is only available in us-east-1, us-west-2, or eu-west-1."
  }
}
